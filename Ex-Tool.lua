...
╔═══════════════════════════════════════════════════════════════╗
║               C L o g g e r  v2.0                            ║
║        Developer Console & Real-Time Monitor                  ║
║                 created by: SofiAkira                         ║
╠═══════════════════════════════════════════════════════════════╣
║  v2.0  • Responsive sizing (mobile / any screen)              ║
║        • Transparent glass + flowing water background inside  ║
║        • Source viewer: viewport-sized, scrollable, draggable ║
║        • Script Logger: auto-detects executor-injected scripts ║
║        • rSpy tab: __namecall hook + OnClientEvent spy        ║
║        • Player Logger: manual ESP on/off toggle              ║
╚═══════════════════════════════════════════════════════════════╝
]]
...
-- ══════════════════════════════════════════════════════════════
-- [03]  RESPONSIVE SIZING  (fits mobile to 4K)
-- ══════════════════════════════════════════════════════════════
...
local TAB_W    = math.floor((PW - 28) / 4) - 3   -- 4 tabs
...
Root.Name = "CLogger"; Root.ResetOnSpawn = false
Root.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Root.IgnoreGuiInset = true; Root.DisplayOrder = 999

local ok = false
if isExec then ok = pcall(function() Root.Parent = game:GetService("CoreGui") end) end
...
local function corner(p, r)
    local c = Instance.new("UICorner", p); c.CornerRadius = r or UDim.new(0,8); return c
end
local function uistroke(p, col, t, tr)
    local s = Instance.new("UIStroke", p)
    s.Color=col or Color3.fromRGB(40,120,220); s.Thickness=t or 1.5; s.Transparency=tr or 0
    return s
end
local function ipad(p, all, l, r, t, b)
    local u = Instance.new("UIPadding", p)
    local v = UDim.new(0, all or 0)
    u.PaddingLeft=l and UDim.new(0,l) or v; u.PaddingRight=r and UDim.new(0,r) or v
    u.PaddingTop=t and UDim.new(0,t) or v; u.PaddingBottom=b and UDim.new(0,b) or v
end
local function ll(p, dir, sp)
    local l = Instance.new("UIListLayout", p)
    l.FillDirection=dir or Enum.FillDirection.Vertical
    l.SortOrder=Enum.SortOrder.LayoutOrder; l.Padding=UDim.new(0,sp or 4); return l
end
local function lbl(p, props)
    local l = Instance.new("TextLabel", p)
    l.BackgroundTransparency=1; l.TextWrapped=true; l.RichText=false
...
local function mkbtn(p, props)
    ...
    b.MouseEnter:Connect(function()
        TweenService:Create(b,TweenInfo.new(0.12),{BackgroundColor3=Color3.new(
            math.min(nc.R+0.06,1),math.min(nc.G+0.06,1),math.min(nc.B+0.08,1))}):Play()
    end)
...
local function argStr(v)
    ...
    elseif t == "string" then
        local s = v:sub(1,55); return '"'..(#v>55 and s.."…" or s)..'"'
    elseif t == "number" then return tostring(math.round(v*1000)/1000)
    elseif t == "boolean" then return tostring(v)
    elseif t == "Instance" then return v.ClassName..' "'..v.Name..'"'
    elseif t == "table" then local n=0; for _ in pairs(v) do n+=1 end; return "{"..n.." keys}"
    ...
    elseif t == "Color3" then return ("C3(%d,%d,%d)"):format(v.R*255,v.G*255,v.B*255)
    elseif t == "Vector2" then return ("V2(%.1f,%.1f)"):format(v.X,v.Y)
    else return t end
end
local function argsStr(...)
    local t={}; for i=1,select("#",...)do t[i]=argStr(select(i,...)) end
...
Toggle.Size=UDim2.new(0,44,0,44); Toggle.Position=UDim2.new(0,14,0.5,-22)
Toggle.BackgroundColor3=Color3.fromRGB(12,48,115); Toggle.Text="CL"
Toggle.TextColor3=Color3.fromRGB(105,192,255); Toggle.TextSize=15
Toggle.Font=Enum.Font.GothamBold; Toggle.BorderSizePixel=0
Toggle.ZIndex=200; Toggle.Visible=false
...
-- ══════════════════════════════════════════════════════════════
-- [08]  MAIN WINDOW  (transparent glass)
-- ══════════════════════════════════════════════════════════════
local Main = Instance.new("Frame", Root)
Main.Name="Main"; Main.Size=UDim2.new(0,PW,0,PH)
Main.Position=UDim2.new(0.5,-PW/2,0.5,-PH/2)
Main.BackgroundColor3=Color3.fromRGB(6,18,52)
Main.BackgroundTransparency=0.28          -- more transparent = glass look
Main.BorderSizePixel=0; Main.ClipsDescendants=true
corner(Main,UDim.new(0,14))

-- Subtle blue-glass shimmer overlay
local Glass = Instance.new("Frame",Main)
Glass.Size=UDim2.new(1,0,1,0); Glass.BackgroundColor3=Color3.fromRGB(55,125,225)
Glass.BackgroundTransparency=0.91; Glass.BorderSizePixel=0; Glass.ZIndex=1
...
task.spawn(function()
    while Main.Parent do
        TweenService:Create(BG,TweenInfo.new(2,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),
            {Color=Color3.fromRGB(58,160,255),Transparency=0.60}):Play(); task.wait(2)
        TweenService:Create(BG,TweenInfo.new(2,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),
            {Color=Color3.fromRGB(18,88,218),Transparency=0.08}):Play(); task.wait(2)
    end
end)

-- ══════════════════════════════════════════════════════════════
-- [09]  INTERNAL FLOWING WATER BACKGROUND
--  Translucent wave layers that oscillate inside the panel.
--  Clipped by Main (ClipsDescendants=true).  ZIndex 2 so they
--  sit above the glass tint (1) but below all UI content (10+).
-- ══════════════════════════════════════════════════════════════
local IW = Instance.new("Frame",Main)
IW.Size=UDim2.new(1,0,1,0); IW.BackgroundTransparency=1
IW.BorderSizePixel=0; IW.ZIndex=2
...
for i,wd in ipairs(WAVE_DEF) do
    ...
    wf.BorderSizePixel=0; wf.ZIndex=2
...
-- ══════════════════════════════════════════════════════════════
-- [10]  BORDER WATER DROPLETS  (waterfall along all 4 edges)
--  ZIndex 3 — above internal waves (2), below UI content (10+)
-- ══════════════════════════════════════════════════════════════
local WL = Instance.new("Frame",Main)
WL.Size=UDim2.new(1,0,1,0); WL.BackgroundTransparency=1
WL.BorderSizePixel=0; WL.ZIndex=3

local DHUES={200,205,210,215,220,225}
local function spawnDroplet(side)
    local d=Instance.new("Frame",WL); d.BorderSizePixel=0; d.ZIndex=4
    local w=math.random(4,9); local h=math.random(10,22); local spd=math.random(14,36)/10
    ...
    d.BackgroundColor3=col; d.BackgroundTransparency=alp
    ...
    local sp,ep
    if side=="L" then
        g.Rotation=90; d.Size=UDim2.new(0,w,0,h)
        sp=UDim2.new(0,0,0,math.random(-h,10)); ep=UDim2.new(0,0,1,h+6)
    elseif side=="R" then
        g.Rotation=90; d.Size=UDim2.new(0,w,0,h)
        sp=UDim2.new(1,-w,0,math.random(-h,10)); ep=UDim2.new(1,-w,1,h+6)
    elseif side=="T" then
        g.Rotation=0; d.Size=UDim2.new(0,h,0,w); spd=spd*0.4
        local sx=math.random(0,PW-h)
        sp=UDim2.new(0,sx,0,0); ep=UDim2.new(0,sx+math.random(8,30),0,w+1)
    elseif side=="B" then
        g.Rotation=0; d.Size=UDim2.new(0,h,0,w); spd=spd*0.4
        local sx=math.random(0,PW-h)
        sp=UDim2.new(0,sx,1,-w); ep=UDim2.new(0,sx+math.random(8,30),1,-1)
    end
    d.Position=sp
...
-- ══════════════════════════════════════════════════════════════
-- [11]  HEADER  (ZIndex 60, above everything)
-- ══════════════════════════════════════════════════════════════
local HH = math.clamp(math.floor(PH*0.10), 42, 54)
local Header = Instance.new("Frame",Main)
Header.Size=UDim2.new(1,0,0,HH); Header.BackgroundColor3=Color3.fromRGB(6,22,70)
Header.BackgroundTransparency=0.10; Header.BorderSizePixel=0; Header.ZIndex=60
...
Logo.Size=UDim2.new(0,HH-14,0,HH-14); Logo.Position=UDim2.new(0,10,0.5,-(HH-14)/2)
Logo.BackgroundColor3=Color3.fromRGB(22,90,195); Logo.BackgroundTransparency=0.28
Logo.BorderSizePixel=0; Logo.ZIndex=61
...
TabBar.Size=UDim2.new(1,-16,0,TABH); TabBar.Position=UDim2.new(0,8,0,HH+2)
TabBar.BackgroundTransparency=1; TabBar.BorderSizePixel=0; TabBar.ZIndex=60
ll(TabBar,Enum.FillDirection.Horizontal,4)

local Div=Instance.new("Frame",Main)
Div.Size=UDim2.new(1,-16,0,1); Div.Position=UDim2.new(0,8,0,HH+TABH+4)
Div.BackgroundColor3=Color3.fromRGB(28,86,195); Div.BackgroundTransparency=0.44
Div.BorderSizePixel=0; Div.ZIndex=60
...
ContentArea.ZIndex=10; ContentArea.ClipsDescendants=true
...
local function switchTab(name)
    for k,p in pairs(tabPanels) do
        p.Visible=false
        local b=tabBtns[k]
        if b then b.BackgroundColor3=C_OFF; b.BackgroundTransparency=0.36; b.TextColor3=CT_OFF end
    end
    if tabPanels[name] then
        tabPanels[name].Visible=true
        local b=tabBtns[name]
        if b then b.BackgroundColor3=C_ON; b.BackgroundTransparency=0; b.TextColor3=CT_ON end
        activeTab=name
    end
end
...
local function makeTab(name,icon,order)
    local b=mkbtn(TabBar,{
        ...
        Text=icon.." "..name,TextColor3=CT_OFF,
        TextSize=FONT_SM,Font=Enum.Font.GothamSemibold,
        LayoutOrder=order,ZIndex=61,TextScaled=false,
    ...
    panel.Visible=false; panel.ZIndex=11
    ipad(panel,6)
    ll(panel,Enum.FillDirection.Vertical,5)

    tabPanels[name]=panel; tabBtns[name]=b
...
local function aiAnalyze(raw)
    local m=raw:lower()
    local out={"🤖  CLogger AI  "..string.rep("─",28),""}
    ...
    elseif m:match("attempt to call a? ?nil") then
        ...
        sep(); add("  ▸ FIX     Check spelling (case-sensitive) and require().")
        add("             Wrap in pcall to isolate the crash.")
    ...
    elseif m:match("infinite yield possible") then
        ...
        add("  ▸ CAUSE   '"..nm.."' may not exist or name is misspelled.")
...
local function addLog(msg,msgType)
    ...
    e.BorderSizePixel=0; e.LayoutOrder=logSeq; e.ZIndex=12
    corner(e,UDim.new(0,6))
    ipad(e,nil,14,7,5,6); ll(e,Enum.FillDirection.Vertical,3)

    local stripe=Instance.new("Frame",e)
    stripe.Size=UDim2.new(0,3,1,0); stripe.BackgroundColor3=meta.sc
    stripe.BorderSizePixel=0; stripe.ZIndex=13; corner(stripe,UDim.new(0,2))

    local tr=Instance.new("Frame",e)
    tr.Size=UDim2.new(1,0,0,16); tr.BackgroundTransparency=1; tr.LayoutOrder=1; tr.ZIndex=13
    local bdg=lbl(tr,{Size=UDim2.new(0,40,1,0),BackgroundColor3=meta.tc,
        BackgroundTransparency=0.48,Text=meta.tag,TextColor3=Color3.fromRGB(235,240,255),
        TextSize=FONT_SM-1,Font=Enum.Font.GothamBold,ZIndex=14})
    bdg.BackgroundTransparency=0.48; corner(bdg,UDim.new(0,3))
    lbl(tr,{Size=UDim2.new(1,-46,1,0),Position=UDim2.new(0,46,0,0),Text=os.date("%H:%M:%S"),
        TextColor3=Color3.fromRGB(68,115,182),TextSize=FONT_SM-1,Font=Enum.Font.Code,
        TextXAlignment=Enum.TextXAlignment.Left,ZIndex=14})

    lbl(e,{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,Text=msg,
        TextColor3=meta.textc,TextSize=FONT_SM,Font=Enum.Font.Code,
        TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=2,ZIndex=13})

    if isErr then
        local ab=mkbtn(e,{Size=UDim2.new(0,100,0,20),LayoutOrder=3,
            BackgroundColor3=Color3.fromRGB(24,72,162),Text="🤖 Assist AI",
            TextColor3=Color3.fromRGB(145,202,255),TextSize=FONT_SM-1,
            Font=Enum.Font.GothamSemibold,ZIndex=13})
        corner(ab,UDim.new(0,5))
        local aiBox=Instance.new("Frame",e)
        aiBox.Size=UDim2.new(1,0,0,0); aiBox.AutomaticSize=Enum.AutomaticSize.Y
        aiBox.BackgroundColor3=Color3.fromRGB(8,26,70); aiBox.BackgroundTransparency=0.06
        aiBox.BorderSizePixel=0; aiBox.LayoutOrder=4; aiBox.Visible=false; aiBox.ZIndex=13
        corner(aiBox,UDim.new(0,6)); ipad(aiBox,7)
        local aitxt=lbl(aiBox,{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,
            TextColor3=Color3.fromRGB(145,208,255),TextSize=FONT_SM-1,Font=Enum.Font.Code,
            TextXAlignment=Enum.TextXAlignment.Left,ZIndex=14})
        local open=false
        ab.MouseButton1Click:Connect(function()
            open=not open
            if open then aitxt.Text=aiAnalyze(msg); aiBox.Visible=true
                ab.Text="🤖 Hide"; ab.BackgroundColor3=Color3.fromRGB(52,26,132)
            else aiBox.Visible=false; ab.Text="🤖 Assist AI"; ab.BackgroundColor3=Color3.fromRGB(24,72,162) end
        end)
    end
end

local consoleClear=mkbtn(consolePanel,{Size=UDim2.new(1,0,0,26),LayoutOrder=0,
...
consoleClear.MouseButton1Click:Connect(function()
    for _,c in ipairs(consolePanel:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
    logSeq=0
end)

LogService.MessageOut:Connect(addLog)

-- Load history (output before CLogger loaded)
task.spawn(function()
    pcall(function()
...
-- Hook executor print/warn so they appear in the console
if isExec then
    pcall(function()
        local _P=getgenv().print; local _W=getgenv().warn
...
local PURP={
    ...
    kill="Kill/Damage",   leader="Leaderboard", door="Door",
    pet="Pets",           cash="Economy",       coin="Currency",
    tween="Tween util",   touch="Touch",        module="Module",
}
...
local function tryGetSource(s)
    ...
    if isExec then
        ...
        if src~="" then return src,"✅ getscriptsource()" end
        pcall(function()
            if type(decompile)=="function" then
                local r=decompile(s); if r and r~="" then src=r end
            end
        end)
        if src~="" then return src,"⚠ decompile() (reconstructed)" end
    end
    -- Metadata fallback
    local lines={}
    ...
    if isExec then
        ...
        w("  Possible: server Script (only LocalScripts are readable client-side),")
    ...
    for _=1,14 do if not cur or cur==game then break end
        table.insert(chain,1,"    "..cur.ClassName..' "'..cur.Name..'"'); cur=cur.Parent end
    for _,line in ipairs(chain) do w(line) end
    w('    └─ '..s.ClassName..' "'..s.Name..'"')
    w(""); w("  ── Children ──")
    local kids=s:GetChildren()
    if #kids==0 then w("  (none)") else
        for _,c in ipairs(kids) do w("  • "..c.ClassName..' "'..c.Name..'"') end end
    w("]]")
    return table.concat(lines,"\n"),"⛔ Metadata only"
end

-- Source popup (viewport-sized, scrollable, draggable on touch+mouse)
local function openSourcePopup(s)
    local src,method=tryGetSource(s)
    local isMeta=src:sub(1,4)=="--[["

    -- Large real source → save file
    if isExec and not isMeta and #src>3800 then
        ...
        nf.Size=UDim2.new(0,math.min(PW-10,360),0,50); nf.Position=UDim2.new(0.5,-170,0,22)
        nf.BackgroundColor3=ok2 and Color3.fromRGB(12,65,25) or Color3.fromRGB(80,20,20)
        nf.BorderSizePixel=0; corner(nf,UDim.new(0,9))
        lbl(nf,{Size=UDim2.new(1,-10,1,0),Position=UDim2.new(0,5,0,0),
            Text=ok2 and ("✅ Saved: "..fname) or "❌ writefile() failed",
            TextColor3=ok2 and Color3.fromRGB(138,255,168) or Color3.fromRGB(255,130,130),
            TextSize=FONT_SM,Font=Enum.Font.GothamSemibold,TextXAlignment=Enum.TextXAlignment.Left})
        task.delay(4,function() pcall(function() ng:Destroy() end) end)
        return
    end

    -- Inline viewer — sized to viewport so it fits any screen
    ...
    vf.Size=UDim2.new(0,vfW,0,vfH); vf.Position=UDim2.new(0.5,-vfW/2,0.5,-vfH/2)
    vf.BackgroundColor3=Color3.fromRGB(5,14,42); vf.BorderSizePixel=0
    corner(vf,UDim.new(0,11)); uistroke(vf,Color3.fromRGB(28,102,205),1.5)

    -- Header (drag zone)
    local VHDRH=54
    local vh=Instance.new("Frame",vf)
    vh.Size=UDim2.new(1,0,0,VHDRH); vh.BackgroundColor3=Color3.fromRGB(10,38,98)
    vh.BorderSizePixel=0; corner(vh,UDim.new(0,11))

    lbl(vh,{Size=UDim2.new(1,-42,0,22),Position=UDim2.new(0,10,0,6),
        Text="📄  "..s:GetFullName(),TextColor3=Color3.fromRGB(155,212,255),
        TextSize=FONT_SM,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,
        TextTruncate=Enum.TextTruncate.AtEnd})

    local mCol=method:match("^✅") and Color3.fromRGB(90,225,125)
           or method:match("^⚠") and Color3.fromRGB(255,205,70)
    ...
    lbl(vh,{Size=UDim2.new(1,-42,0,16),Position=UDim2.new(0,10,0,32),
        ...
        TextSize=FONT_SM-1,Font=Enum.Font.GothamSemibold,TextXAlignment=Enum.TextXAlignment.Left})

    local vcl=mkbtn(vh,{Size=UDim2.new(0,26,0,26),Position=UDim2.new(1,-32,0.5,-13),
        BackgroundColor3=Color3.fromRGB(158,32,32),Text="✕",
        TextColor3=Color3.fromRGB(255,255,255),TextSize=FONT_SM,Font=Enum.Font.GothamBold,ZIndex=5})
    corner(vcl,UDim.new(0,5)); vcl.MouseButton1Click:Connect(function() vg:Destroy() end)

    -- Scrollable source body — takes remaining height
    local vsf=Instance.new("ScrollingFrame",vf)
    vsf.Size=UDim2.new(1,-10,1,-(VHDRH+6))
    vsf.Position=UDim2.new(0,5,0,VHDRH+4)
    vsf.BackgroundColor3=Color3.fromRGB(3,9,26); vsf.BorderSizePixel=0
    vsf.ScrollBarThickness=4; vsf.ScrollBarImageColor3=Color3.fromRGB(42,125,210)
    vsf.CanvasSize=UDim2.new(0,0,0,0); vsf.AutomaticCanvasSize=Enum.AutomaticSize.Y
    corner(vsf,UDim.new(0,7)); ipad(vsf,6)
    lbl(vsf,{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,
        Text=src,TextColor3=Color3.fromRGB(160,212,255),TextSize=FONT_SM,
    ...
    UserInputService.InputChanged:Connect(function(i)
        if vd.on and(i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
            local d=i.Position-vd.ms
            vf.Position=UDim2.new(vd.sp.X.Scale,vd.sp.X.Offset+d.X,vd.sp.Y.Scale,vd.sp.Y.Offset+d.Y)
        end
    end)
...
local function addScriptEntry(s)
    ...
    knownScripts[s]=true; sSeq+=1
    local tc=SCC[s.ClassName] or Color3.fromRGB(175,175,195)
    local ef=Instance.new("Frame",scriptPanel)
    ef.Size=UDim2.new(1,0,0,56); ef.BackgroundColor3=Color3.fromRGB(8,24,65)
    ef.BackgroundTransparency=0.14; ef.BorderSizePixel=0
    ef.LayoutOrder=sSeq+10; ef.ZIndex=12
    ...
    local bdg=lbl(ef,{Size=UDim2.new(0,90,0,15),BackgroundColor3=tc,
        ...
        TextColor3=Color3.fromRGB(235,242,255),TextSize=FONT_SM-1,Font=Enum.Font.GothamBold,ZIndex=13})
    bdg.BackgroundTransparency=0.50; corner(bdg,UDim.new(0,4))
    lbl(ef,{Size=UDim2.new(1,-76,0,18),Position=UDim2.new(0,0,0,18),
        Text=s.Name,TextColor3=Color3.fromRGB(195,228,255),TextSize=FONT_SM,
    ...
    lbl(ef,{Size=UDim2.new(1,-76,0,13),Position=UDim2.new(0,0,1,-14),
        ...
        TextSize=FONT_SM-1,Font=Enum.Font.Code,TextXAlignment=Enum.TextXAlignment.Left,
        TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=13})
...
local scanInfo=lbl(scriptPanel,{Size=UDim2.new(1,0,0,14),LayoutOrder=1,
    Text="Press Scan — auto-detects new executor scripts every 2s.",
...
local scanBtn=mkbtn(scriptPanel,{Size=UDim2.new(1,0,0,28),LayoutOrder=0,
    ...
    TextColor3=Color3.fromRGB(122,255,165),TextSize=FONT_SM,Font=Enum.Font.GothamBold,ZIndex=12})
...
scanBtn.MouseButton1Click:Connect(function()
    scanBtn.Text="⏳  Scanning…"; scanBtn.BackgroundColor3=Color3.fromRGB(35,65,16)
    for _,c in ipairs(scriptPanel:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
    sSeq=0; knownScripts={}
    task.spawn(function()
        doScan()
        local count=0; for _ in pairs(knownScripts) do count+=1 end
        scanInfo.Text=string.format("✔ %d script(s) found. Auto-detecting new ones…",count)
        scanBtn.Text=string.format("🔍  Rescan  (%d)",count)
        scanBtn.BackgroundColor3=Color3.fromRGB(14,86,44)
    end)
end)

-- Auto-detect newly executed scripts every 2 seconds
task.spawn(function()
    while Root.Parent do
...
-- ══════════════════════════════════════════════════════════════
-- [16]  PLAYER LOGGER TAB  (with ESP toggle)
-- ══════════════════════════════════════════════════════════════
local playerPanel=makeTab("Players","👤",3)
local pData,pCards,pESP,pSeq={},{},{},0
local espEnabled=true   -- global ESP on/off

-- ESP toggle button (top of tab)
local espToggleBtn=mkbtn(playerPanel,{Size=UDim2.new(1,0,0,28),LayoutOrder=0,
...
espToggleBtn.MouseButton1Click:Connect(function()
    ...
    if espEnabled then
        ...
        -- Clear all active ESP immediately
...
local function buildESP(player)
    ...
    for _,p in ipairs(char:GetDescendants()) do
        if p:IsA("BasePart") then
            ...
            sb.Adornee=p; sb.Color3=Color3.fromRGB(255,52,52)
            sb.LineThickness=0.055; sb.SurfaceTransparency=0.72
    ...
    bg.BackgroundTransparency=0.40; bg.BorderSizePixel=0; corner(bg,UDim.new(0,7))
    local infoL=lbl(bg,{Size=UDim2.new(1,-8,1,-6),Position=UDim2.new(0,4,0,3),
        TextColor3=Color3.fromRGB(255,192,192),TextSize=12,Font=Enum.Font.GothamSemibold,
...
local function makeCard(player)
    if pCards[player] then return end; pSeq+=1
    ...
    lbl(card,{Size=UDim2.new(0.6,0,0,20),Text="👤  "..player.DisplayName,
        TextColor3=Color3.fromRGB(192,232,255),TextSize=FONT_SM,Font=Enum.Font.GothamBold,
    ...
    lbl(card,{Size=UDim2.new(0.6,0,0,14),Position=UDim2.new(0,0,0,21),
        ...
        TextSize=FONT_SM-2,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=13})
    local statusL=lbl(card,{Size=UDim2.new(1,0,0,16),Position=UDim2.new(0,0,0,37),
        Text="● Normal",TextColor3=Color3.fromRGB(68,218,108),TextSize=FONT_SM-1,
    ...
    local distL=lbl(card,{Size=UDim2.new(0.6,0,0,14),Position=UDim2.new(0,0,0,55),
        Text="📍  —",TextColor3=Color3.fromRGB(148,195,255),TextSize=FONT_SM-1,
...
local function onGround(hrp)
    local rp=RaycastParams.new(); rp.FilterDescendantsInstances={hrp.Parent}
...
RunService.Heartbeat:Connect(function(dt)
    hbAcc+=dt; if hbAcc<0.10 then return end; hbAcc=0
    ...
    for _,player in ipairs(Players:GetPlayers()) do
        ...
        local data=pData[player]; local card=pCards[player]; if not card then continue end
        local char=player.Character
        local hrp=char and char:FindFirstChild("HumanoidRootPart")
        local hum=char and char:FindFirstChildWhichIsA("Humanoid")
        if not hrp then
            card.statusL.Text="⭕ Not spawned"; card.statusL.TextColor3=Color3.fromRGB(148,148,160)
            card.distL.Text="📍 N/A"; clearESP(player); continue
        end
        ...
        if hum then
            ...
            card.hpBar.BackgroundColor3=Color3.new(math.clamp((1-r)*2,0,1),math.clamp(r*1.4,0,1),0.04)
        end
        local dist=myHRP and math.round((myHRP.Position-pos).Magnitude) or -1
        if dist>=0 then
            card.distL.Text=("📍 %d studs"):format(dist); card.distL.TextColor3=dCol(dist)
            ...
            elseif not espEnabled or dist>=24 then
                clearESP(player)
                card.espDot.BackgroundColor3=espEnabled and Color3.fromRGB(44,44,66) or Color3.fromRGB(80,22,22)
            end
        end
        if data.lastPos then
            data.afkTimer=(pos-data.lastPos).Magnitude<0.5 and data.afkTimer+0.10 or 0
        end
        data.lastPos=pos
        table.insert(data.posHistory,{p=pos,t=now})
        while #data.posHistory>20 do table.remove(data.posHistory,1) end
        local flags,flagCol={},Color3.fromRGB(68,218,108)
        if data.afkTimer>=300 then table.insert(flags,"💤 AFK 5min+"); flagCol=Color3.fromRGB(168,168,182)
        elseif data.afkTimer>=120 then table.insert(flags,"💤 AFK 2min+"); flagCol=Color3.fromRGB(178,178,190) end
        if hum then
            local state=hum:GetState()
            if not onGround(hrp) and state~=Enum.HumanoidStateType.Freefall
        ...
        if #flags==0 then card.statusL.Text="● Normal"; card.statusL.TextColor3=Color3.fromRGB(68,218,108)
        else card.statusL.Text="● "..table.concat(flags,"  |  "); card.statusL.TextColor3=flagCol end
    end
...
-- ══════════════════════════════════════════════════════════════
-- [17]  rSPY TAB
--  • OnClientEvent  → catches all Server→Client remote fires
--  • __namecall hook → catches Client→Server FireServer /
--    InvokeServer calls (executor only, graceful fallback)
-- ══════════════════════════════════════════════════════════════
local rspyPanel=makeTab("rSpy","📡",4)
local rspySeq=0
local rspyActive=true
local rspyConns={}   -- [remote] = connection for OnClientEvent
...
local rspyInfoL=lbl(rspyPanel,{Size=UDim2.new(1,0,0,13),LayoutOrder=1,
    Text=isExec and "Hooking FireServer+InvokeServer (C→S) and OnClientEvent (S→C)."
...
local function addRSpyEntry(remoteName,remotePath,direction,...)
    ...
    -- Row 1: direction badge + time + remote name
    ...
    local dirBdg=lbl(r1,{Size=UDim2.new(0,110,1,0),BackgroundColor3=dc,
        BackgroundTransparency=0.50,Text=direction,
        TextColor3=Color3.fromRGB(238,245,255),TextSize=FONT_SM-1,Font=Enum.Font.GothamBold,ZIndex=14})
...
-- Hook OnClientEvent for all existing remotes (S→C direction)
local function hookRemote(remote)
    ...
    elseif remote:IsA("RemoteFunction") then
        -- Wrap OnClientInvoke to log S→C invocations
        pcall(function()
...
-- Executor __namecall hook to capture C→S direction (FireServer / InvokeServer)
if isExec then
    pcall(function()
        ...
        -- Use newcclosure if available (prevents detection / errors in some games)
...
-- ══════════════════════════════════════════════════════════════
-- [18]  DRAG  (header grab — mouse + touch)
-- ══════════════════════════════════════════════════════════════
...
UserInputService.InputChanged:Connect(function(i)
    if drag.on and(i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
        ...
        Main.Position=UDim2.new(drag.sp.X.Scale,drag.sp.X.Offset+d.X,drag.sp.Y.Scale,drag.sp.Y.Offset+d.Y)
    end
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
...
-- ══════════════════════════════════════════════════════════════
switchTab("Console")
print(("[CLogger v2.0]  ✅  Loaded  %dx%d  — created by SofiAkira"):format(PW,PH))
